...
func insert_item(item):
	...
	if item_slot == null :
		return false
	if !slot.name.begins_with(item_slot.substr(0,4)) and item_slot != null:
		print('No puedes vestir un casco como zapatos')
		return false
	if items[slot.name] != null:
		if slot.name.begins_with(item_slot.substr(0,4)) :
			var in_slot_item = grab_item(item_pos) # items[slot.name]
			items[slot.name] = item
			item.set_meta('occupied_slot', slot.name )
			in_slot_item.set_meta('occupied_slot', null)
			BackPack.Back_Pack[item.name].occupied_slot = str(slot.name)
...
static func load_equiped_slots():
	...
	for reload_it in items :
		#print(reload_it)
		#print(items[reload_it])
		if  is_instance_valid(items[reload_it]) :
...
static func delete_return_items():
	for it_to_del in items  :
			if items[it_to_del] and is_instance_valid(items[it_to_del]):
